...
import { Copy, Eye, EyeOff, Trash2, Edit2, Check, Code } from 'lucide-react';
...
export function ApisList({ search }: { search: string }) {
  ...
  const entries = vaultData?.apis.filter(p => 
...
function ApiRow({ entry, onEdit, onDelete }: { entry: VaultEntry, onEdit: () => void, onDelete: () => void }) {
